    ...
    <div className="max-w-7xl mx-auto py-8">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100 flex items-center">
          <Users className="mr-3 w-6 h-6 text-indigo-600 dark:text-indigo-400" />
          Employee Management
        </h2>
        <button
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-700">
                <th className="p-4 font-semibold text-slate-600 dark:text-slate-300">Emp ID</th>
                <th className="p-4 font-semibold text-slate-600 dark:text-slate-300">Name / Contact</th>
                <th className="p-4 font-semibold text-slate-600 dark:text-slate-300">Assigned Villages</th>
            ...
            <tbody>
              {employees.length === 0 ? (
                ...
                employees.map((emp) => (
                  <tr key={emp._id} className="border-b border-slate-100 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                    <td className="p-4 text-slate-800 dark:text-slate-200 font-medium whitespace-nowrap">{emp.employeeId || 'N/A'}</td>
                    <td className="p-4">
                      <div className="font-medium text-slate-800 dark:text-slate-200">{emp.name || 'N/A'}</div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {emp.assignedVillages && emp.assignedVillages.length > 0 ? (
                          emp.assignedVillages.map((locId, idx) => (
                            <span key={idx} className="inline-flex items-center px-2 py-1 rounded bg-indigo-50 text-indigo-700 text-xs font-medium dark:bg-indigo-900/30 dark:text-indigo-300">
                              Village ID: {locId.toString().slice(-4)}
                              <button onClick={() => removeVillage(emp._id, emp.assignedVillages, locId)} className="ml-1.5 text-indigo-400 hover:text-indigo-900"><X className="w-3 h-3" /></button>
                            </span>
                          ))
                        ) : (
                          <span className="text-slate-400 text-xs italic">No villages assigned</span>
                        )}
                        <button 
                          onClick={() => openAssignModal(emp)} 
                          className="inline-flex items-center px-2 py-1 rounded border border-dashed border-indigo-300 text-indigo-600 hover:bg-indigo-50 hover:border-indigo-400 transition-colors text-xs font-medium"
                        >
                    ...
                    <td className="p-4 text-right whitespace-nowrap">
                      <button onClick={() => openEditModal(emp)} className="p-2 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-lg transition-colors mr-2" title="Edit Employee">
                        <Edit className="w-4 h-4" />
                      </button>
                      <button onClick={() => openDeleteModal(emp)} className="p-2 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition-colors" title="Delete Employee">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input
                  type="text"
                  required
                  className="w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-indigo-500 outline-none transition-shadow"
                  value={formData.name}
              ...
              <div className="pt-4 flex justify-end gap-3 border-t border-slate-200 dark:border-slate-700">
                <button type="button" onClick={() => setIsModalOpen(false)} className="px-5 py-2 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-colors font-medium">Cancel</button>
                <button type="submit" className="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg shadow-sm transition-colors font-medium">
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-sm overflow-hidden p-6 text-center border border-slate-200 dark:border-slate-700">
            ...
            <div className="flex justify-center gap-3">
              <button onClick={() => setIsDeleteModalOpen(false)} className="flex-1 px-4 py-2 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-colors font-medium">Cancel</button>
            ...
            <div className="p-6 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/30 flex justify-end space-x-3">
              ...
              <button 
                ...
                className={`px-6 py-2 rounded-xl text-white font-medium flex items-center transition-all ${
                  !selLocationId ? 'bg-slate-400 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-700 shadow-md'
                }`}
              >
